...
# Admin user IDs
admin_id = {"7915760293"}  # Replace with actual admin ID
...
def generate_key(length=10):
    ...
    return f"RIYAZ-VIP-{key.upper()}"  # Ensure key is in uppercase
...
def generate_key_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗔𝗱𝗺𝗶𝗻 @RIYAZ_5U"
...
def execute_attack(user_id, target, port, time):
    ...
    try:
        ...
        full_command = f"./RIYAZ {target} {port} {time}"  # Replace with actual attack command
